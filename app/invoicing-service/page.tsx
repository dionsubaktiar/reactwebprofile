...
const MainPageInvoice = () => {
  ...
  const [apiResponse, setApiResponse] = useState<Invoice[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setApiResponse(response.data.data.data);
      } catch (error) {
        ...
        <table className="min-w-full table-auto overflow-visible">
          <thead>
            <tr className="bg-customGreen-dark text-honeyDew">
              ...
              <th className="px-4 py-2 text-left">Jenis Service</th>
          ...
          <tbody>
            {apiResponse.map((invoice) => (
              <tr key={invoice.id} className="border-t">
                <td className="px-4 py-2 text-nowrap">{invoice.units.nopol}</td>
